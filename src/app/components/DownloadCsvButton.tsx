...
export default function DownloadCsvButton({ accessToken }: Props) {
  const handleDownload = async () => {
    try {
      // Prefer Supabase Functions endpoints (use session token if available, otherwise anon key); fall back to local/static files
      ...
      if (cadetsCount === 0 && attendanceCount === 0 && pointsCount === 0) {
        console.warn('DownloadCsvButton: no data found', { cadetsCount, attendanceCount, pointsCount })
        alert(`No cadet, attendance, or points data found to export. Found counts — cadets: ${cadetsCount}, attendance: ${attendanceCount}, points: ${pointsCount}.\n\nCheck the browser DevTools Network tab for failing requests to your Supabase functions (ensure the site is logged in or publicAnonKey allows access).`)
        return
      }
      ...
      if (topCadet) {
        ...
        alert(`Exported CSVs. Top cadet: ${topCadet.name} (${topCadet.totalPoints} points)`) 
      } else {
        alert('Exported CSVs (no top cadet found)')
      }

    } catch (err) {
      console.error(err)
      alert('Failed to download CSVs. See console for details.')
    }
  }
